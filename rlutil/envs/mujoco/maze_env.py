...
from rlutil.envs.mujoco.maze_env_utils import construct_maze
from rlutil.envs.mujoco.maze_env_utils import ray_segment_intersect, point_distance
...
class MazeEnv(wrappers.Wrapper):
    MODEL_FILE = None
    MODEL_CLS = None
    ORI_IND = None
    ...
    def __init__(
        ...
        model_file = self.__class__.MODEL_FILE
        if model_file is None:
            raise "MODEL_FILE unspecified!"
        ...
        for counter, geom in enumerate(geoms):
            if 'name' not in geom.attrib:
                # generate a name
                geom.attrib['name'] = 'unnamed_%d' % counter
        ...
        super(MazeEnv, self).__init__(inner_env)  # here is where the robot env will be initialized
    ...
    def step(self, action):
        ...
        structure_x = (x + self._init_torso_x) / self.MAZE_SIZE_SCALING + 0.5
        structure_y = (y + self._init_torso_y) / self.MAZE_SIZE_SCALING + 0.5
        structure_reward = self.MAZE_REWARD[int(structure_y), int(structure_x)]
